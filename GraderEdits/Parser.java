...
public class Parser {
	...
	*/
	...
	public List<ClassOrInterfaceType> implementedTypes;

	public List<ClassOrInterfaceType> extendedTypes;
	...
	*/
	...
	public boolean parse(String file) throws Exception {
			...
			this.coid = childNodes;
			isInterface = childNodes.isInterface();
			implementedTypes = childNodes.getImplementedTypes();
			extendedTypes = childNodes.getExtendedTypes();
	...
	*/
	...
	public CompilationUnit getCompilationUnit(File f) {
		try {
			JavaParser parser = new JavaParser();
			ParseResult<CompilationUnit> result = parser.parse(ParseStart.COMPILATION_UNIT, Providers.provider(f));
	...
	public ClassOrInterfaceDeclaration getClassFromFile(File f, CompilationUnit cu) {
		try {
			int start = f.getPath().lastIndexOf("/") + 1;
			if (start == -1) start = 0;
			return cu.getClassByName(f.getPath().substring(start, f.getPath().length()-5)).get();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public CallableDeclaration findMethodOrConstructor(ClassOrInterfaceDeclaration coid, String signature) {
		try {
			for (Node n : coid.getChildNodes()) {
				if (n instanceof MethodDeclaration) {
					...
					ConstructorDeclaration m = (ConstructorDeclaration)n;
					if (m.getDeclarationAsString(false, false, false).equals(signature)) {
						return m;
					}
				}
	...
	public void replaceMethodOrConstructorBody(CallableDeclaration current, CallableDeclaration replacement) {
		if (current instanceof MethodDeclaration && replacement instanceof MethodDeclaration) {
			MethodDeclaration c = (MethodDeclaration)current;
			MethodDeclaration r = (MethodDeclaration)replacement;
			c.setBody(r.getBody().get());
			c.setParameters(r.getParameters());
		} else if (current instanceof ConstructorDeclaration && replacement instanceof ConstructorDeclaration) {
			ConstructorDeclaration c = (ConstructorDeclaration)current;
			ConstructorDeclaration r = (ConstructorDeclaration)replacement;
			c.setBody(r.getBody());
			c.setParameters(r.getParameters());
		}
	...
	*/
	...
	public File replaceMethod(String signature, String file) {
		...
		replaceMethodOrConstructorBody(current, replacement);
