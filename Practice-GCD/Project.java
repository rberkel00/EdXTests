	...
	public static void main(String[] args) {
		...
		try {
			...
			if (!TestCase.compile(new File("Practice.java"))) {
				...
				tests[1].setResult(TestCase.runMain(".", "Practice", null, "(?s).*8.*1.*20.*"));
			}

			Parser parser = new Parser();
			if(parser.parse("Practice.java")){
				List<Node> nodes = parser.findPieces("int greatestCommonDivisor(int, int)");
				for (Node n : nodes) {
					if (n instanceof MethodCallExpr) {
						if(((MethodCallExpr)(n)).getName().asString().equals("greatestCommonDivisor")){
