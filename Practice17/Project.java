	...
	public static void main(String[] args) {
		...
		try {
			...
			if (!TestCase.compile(new File("Practice.java"))) {
				...
				tests[2].setResult(TestCase.runMain(".", "Practice", null, "(?s).*New York is in the Atlantic region.*New Mexico is in the Southwest region.*Louisiana is in the Southeast region.*Kansas is in the Great Plains region.*Oregon is in the Pacific region.*Indiana is in the Midwest region.*Colorado is in the Mountain region.*Virginia is in the Atlantic region.*The Shire is in the Unknown region.*"));
			}
