...
public class Project {
	public static void main(String[] args) {
		...
		try {
			TestCase[] tests = {
				new TestCase("Test 1", "ltr is declared as a char", "Completion", 1),
				new TestCase("Test 2", "ltr is given the value 80", "Completion", 1)
			};

			if (!TestCase.compile(new File("Practice.java"))) {
				System.out.println("Practice.java does not compile.");
			} else {
				tests[1].setResult(TestCase.runMain(".", "Practice", null, "(?s).*old.Purdue.*"));
			}
					...
					for (Expression ex : exs) {
						if (ex instanceof VariableDeclarationExpr) {
							...
							for (VariableDeclarator vd : v.getVariables()) {
								if (vd.getName().asString().equals("ltr") && vd.getType().asString().equals("char")) {
