...
public class Parser {
	...
	*/
	...
	public File[] replaceAllMethods(String[] sigs, String work) throws Exception {
	...
	*/
	...
	public List<Node> getNodes(String method) throws Exception {
		...
		for (MethodDeclaration m : methods) {
			if (m.getDeclarationAsString(false, false, false).equals(method)) {
				...
				for (Statement s : statements) {
					expressions.addAll(getChildNodes(s));
				}
			}
		}
		for (ConstructorDeclaration m : constructors) {
	...
	*/
	...
	public List<Node> getChildNodes(Node ex) throws Exception {
		List<Node> e = new ArrayList<Node>();
		e.add(ex);
		if (ex instanceof ArrayAccessExpr) {
			ArrayAccessExpr ae = (ArrayAccessExpr)ex;
			e.addAll(getChildNodes(ae.getName()));
			e.addAll(getChildNodes(ae.getIndex()));
		}
			...
			if (ae.getExpressionBody().isPresent()) {
				e.addAll(getChildNodes(ae.getExpressionBody().get()));
			}
			e.addAll(getChildNodes(ae.getBody()));
		}
			...
			if (ae.getScope().isPresent()) {
				e.addAll(getChildNodes(ae.getScope().get()));
			}
		}
			...
			if (ae.getClassExpr().isPresent()) {
				e.addAll(getChildNodes(ae.getClassExpr().get()));
			}
		}
				...
				if (n.size() > 0) {
					if (n.get(0) instanceof VariableDeclarator) {
						...
						if (vd.getInitializer().isPresent()) {
							e.addAll(getChildNodes(vd.getInitializer().get()));
						}
					}
				}
			}
			...
			e.addAll(getChildNodes(as.getCheck()));
			if (as.getMessage().isPresent()) {
				e.addAll(getChildNodes(as.getMessage().get()));
			}
		}
			...
			DoStmt ds = (DoStmt)ex;
			e.addAll(getChildNodes(ds.getBody()));
			e.addAll(getChildNodes(ds.getCondition()));
		}
			...
			for (Expression ee : es.getArguments()) {
				e.addAll(getChildNodes(ee));
			}
			if (es.getExpression().isPresent()) {
				e.addAll(getChildNodes(es.getExpression().get()));
			}
		}
		else if (ex instanceof ExpressionStmt) {
			ExpressionStmt es = (ExpressionStmt)ex;
			e.addAll(getChildNodes(es.getExpression()));
		}
		else if (ex instanceof ForeachStmt) {
			ForeachStmt fs = (ForeachStmt)ex;
			e.addAll(getChildNodes(fs.getBody()));
			e.addAll(getChildNodes(fs.getIterable()));
			e.addAll(getChildNodes(fs.getVariable()));
		}
		else if (ex instanceof ForStmt) {
			ForStmt fs = (ForStmt)ex;
			e.addAll(getChildNodes(fs.getBody()));
			if (fs.getCompare().isPresent()) {
				e.addAll(getChildNodes(fs.getCompare().get()));
			}
			...
			IfStmt is = (IfStmt)ex;
			e.addAll(getChildNodes(is.getCondition()));
			if (is.getElseStmt().isPresent()) {
				e.addAll(getChildNodes(is.getElseStmt().get()));
			}
			e.addAll(getChildNodes(is.getThenStmt()));
		}
		else if (ex instanceof LabeledStmt) {
			LabeledStmt ls = (LabeledStmt)ex;
			e.addAll(getChildNodes(ls.getStatement()));
		}
			...
			e.addAll(getChildNodes(ws.getBody()));
			e.addAll(getChildNodes(ws.getCondition()));
		}
		return e;
	}
}
