	...
	public static void main(String[] args) {
		...
		try {
			TestCase[] tests = {
				...
				new TestCase("Test 2", "One other loop is created", "Completion", 1),
			...
			if (p.parse("Practice.java")) {
				...
				List<Node> snodes = p.findPieces("long insertion(ArrayList<Comparable>)");
				for (Node n : mnodes) {
					if (n instanceof MethodCallExpr) {
						MethodCallExpr m = (MethodCallExpr)n;
						if (m.getName().asString().equals("insertion")) {
